  ...
  <div class="w-full h-full flex flex-col justify-start items-center gap-2">
    ...
    <div class="w-full h-96 overflow-y-auto overflow-x-hidden flex flex-col gap-1">
      <div
        ...
        v-for="t in projectsStore.projectTypes"
        :key="t.id"
      >
...
const selectedType = ref<ProjectType | undefined>()
const newType = ref<ProjectType>({})

function addType() {
  if (newType.value.label) {
    projectsStore.addProjectType(newType.value).then(() => {
      newType.value = {}
    })
  }
}
...
async function deleteType(id: number) {
  ...
  if (confirmed) {
    await projectsStore.deleteProjectType(id)
  }
}

function editType() {
  if (selectedType.value) {
    projectsStore.editProjectType(selectedType.value).then(() => {
...
onMounted(async () => {
  await projectsStore.getAllProjectType()
})
</script>
